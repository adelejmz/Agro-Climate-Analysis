...
# ✅ Compute the monthly average solar radiation for each season
seasonal_avg_solar = weather_data.groupby(["Season", "Month-Order"])["Solar Radiation"].mean().unstack(level=0)

# ✅ Use the same style as the humidity plot
plt.style.use('ggplot')
plt.figure(figsize=(10, 6))

# ✅ Plot each season using the same colors and markers as before
for season in seasonal_avg_solar.columns:
    plt.plot(
        seasonal_avg_solar.index, seasonal_avg_solar[season], 
        marker='o', markersize=8, linewidth=2, 
        label=f"{season} Avg Solar Radiation (W/m²)"
    )

# ✅ Customize x-axis labels (Only Nov-May)
plt.xticks(seasonal_avg_solar.index, ["Nov", "Dec", "Jan", "Feb", "Mar", "Apr", "May"], fontsize=12)
plt.xlabel('Growing Season Month', fontsize=14, fontweight='bold')

# ✅ Customize y-axis
plt.ylabel('Avg Solar Radiation (W/m²)', fontsize=14, fontweight='bold')

# ✅ Update Title
plt.title('Monthly Average Solar Radiation in Obregon, Mexico', fontsize=16, fontweight='bold')
...
# ✅ Print final computed seasonal average solar radiation
print("Final computed seasonal average solar radiation:")
print(seasonal_avg_solar)
